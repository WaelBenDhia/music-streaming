	...
	return func(h http.Handler) http.Handler {
		return http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
			...
			if err != nil {
				...
				s.errorLog.Printf("requestParsingMiddleware: error reading from body: %v", err)
				return
			}
			if err := r.Body.Close(); err != nil {
				w.WriteHeader(500)
				w.Header().Set("Content-Type", "text/plain")
				s.errorLog.Printf("requestParsingMiddleware: error in closing body: %v", err)
				return
			}
			if err := json.Unmarshal(body, v); err != nil {
				w.WriteHeader(400)
				s.errorLog.Printf("requestParsingMiddleware: error unmarshaling from body: %v", err)
				return
			}
